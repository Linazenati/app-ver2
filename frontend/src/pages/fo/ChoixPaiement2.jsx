...
const ChoixPaiement = () => {
    ...
    // 🔥 Modification ici : on récupère l'ID visa depuis l'URL (au lieu d'assurance)
    const { visaId } = useParams();
    console.log("ID visa depuis l'URL :", visaId);
    ...
    const handleConfirm = async () => {
        if (formData.methodePaiement === "dahabia") {
            ...
            const paymentData = {
                ...
                mode: "EDAHABIA",
                visaId: visaId // 🔥 Utilisation de visaId ici
            };
            ...
            try {
                const paymentData = {
                    visaId: visaId,
                    reservationId: null // explicitement null si non utilisé
                };
                const response = await stripeService.initiatePayment(paymentData, token); // 🔥 Utilisation de visaId ici aussi
                const paymentLink = response.data.lien_paiement;
