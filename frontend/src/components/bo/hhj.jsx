...
useEffect(() => {
    ...
    console.log("🎯 Modal visible, on charge les quotas");
    async function fetchQuota() {
      try {
        // Simule la récupération du quota (remplace par ton vrai service)
        const response = await throttleService.getQuota();
        console.log("📊 Quota récupéré :", response.data);
        setQuotaMinute(response.data.recupMinuteRestante);
        setQuotaHeure(response.data.recupHeureRestante);
        setQuotaJour(response.data.recupJourRestante);
      } catch (error) {
...
>
  ...
  return (
    ...
    <div style={{ display: "flex", justifyContent: "space-around", marginBottom: 20 }}>
      <CircleQuota
        value={quotaMinute}
        max={4}  // valeur fixe ou dynamique (à ajuster)
        label="Recuperation possible par minute"
      />
      <CircleQuota
        value={quotaHeure}
        max={10}  // idem
        label="Recuperation possible par heure"
      />
      <CircleQuota
        value={quotaJour}
        max={50}  // idem
        label="Recuperation possible par jour"
      />
    </div>
      <Table
